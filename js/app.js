...
function renderHomework(container, weeks) {
  ...
  if (!withHw.length) {
    card.appendChild(el("p", { class: "empty", text: "아직 등록된 과학 숙제가 없습니다." }));
...
function renderQuiz(container) {
  ...
  select(QUIZ_CATEGORIES.some((c) => c.id === selQuizCat) ? selQuizCat : QUIZ_CATEGORIES[0].id);
...
// blob PDF를 새 탭에 열면 표시 대신 다운로드해 버린다. 그런 곳에서는 내장 뷰어
// (js/pdfviewer.js)로 페이지 안에서 직접 그려서 보여주고, 그마저 실패하면
// 저장 + 안내로 폴백한다. 새 탭은 팝업 차단을 피하려고 복호화 전(터치 직후)에 미리 연다.
function fileRow({ title, metaText, entry, key }) {
  ...
  const open = async (mode) => {
    ...
    try {
      ...
      } else if (mode === "view" && isPdf) {
        // 새 탭 표시 불가(뷰어 없음) → 내장 뷰어로 페이지 안에서 표시
        try {
          await showPdfOverlay(blob, fileName);
        } catch (e) {
          console.error(e);
          saveAs(url);
          toast("이 브라우저에서는 파일을 표시할 수 없어 저장했습니다. 저장된 파일을 열어 보세요.", "ok");
        }
      } else if (mode === "view") {
        // PDF가 아닌데 새 탭이 막힌 경우(팝업 차단) → 저장 + 안내
        saveAs(url);
        toast("새 탭이 차단되어 파일을 저장했습니다. 저장된 파일을 열어 보세요.", "ok");
      } else {
        saveAs(url);
      }
    } catch (e) {
...
function renderTeacherHomework(container, weeks) {
  const { teacher } = session;
  const card = el("div", { class: "card" }, [el("h2", { text: "과학 숙제 현황" })]);
  ...
  card.appendChild(
    el("p", { class: "hint", text: "주차(수업)별 과학 숙제 완료율입니다. 최신 주차부터 — 옆으로 밀어서 지난 주차를 보세요." })
  );
...
function renderTeacherMathHomework(container) {
  ...
  if (anyHold || anyNA) {
    const parts = [];
    if (anyHold) parts.push("◌ = 결석 등으로 확인 전 (비율 계산에서 제외)");
    if (anyNA) parts.push("－ = 수강 전·수강 중단 등 해당 없음 (비율 계산에서 제외)");
    card.appendChild(el("p", { class: "hint", text: parts.join(" · ") }));
  }
  container.appendChild(card);
}
