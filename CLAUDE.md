...
2. 학원별 → 주차별(그 날짜가 속한 월~일 주간)로 학생 O/X를 표로 정리해 채팅으로 보여준다.
3. 포털 입력 규칙을 함께 안내한다: 해당 주차 선택 → 수학 숙제 카드의 **'수학 수업일'** 칸에
   시트의 날짜 입력 → **O → ✓ 했음, X → 빈칸(안 함)**, 시트 빈칸 → 선생님 판단
   (◌ 확인 전 / － 해당 없음 — 신규 학생 수강 전·드랍은 자동 － 처리됨).
